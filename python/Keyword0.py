...
def AnalyzeText(loc):
    ...
    # print(main_keyword,111)
    # title_keyword=ExtractTitle(lines[0])
    ...
    for i in range(len(analyze_res)):
        ...
        for j in range(len(analyze_res[i])):
            if analyze_res[i][j][1] in ['NNG','NNP','NNB','NR','NP','VV','VA','SW','SL','SN','XSN','MM','EC','NNBC','XPN'] and analyze_res[i][j][0].lower() not in ['있','사용','실험','때','연구','우리','이용','이용한','확인','결과','번째','번','년','문제','정의','므로','그림','사진','동안','처리','방법','각자','자신','여','of','and','for','was','is','the','to','than','in','fig','self','then','by','this','as','let','we','which','wiki','kr','with','that','on','org','https','has','it','&']:
...
def ExtractTitle(title):
    ...
    temp=[]
    for i in a:
        if i[1] in ['NNG','NNP','NNB','NR','NP','VV','VA','SW','SL','SN','XSN','MM','EC','NNBC','XPN'] and i[0] not in ['있','사용','실험','때','연구','of','우리','이용','이용한','and','for','was','is','the','to','than','in','fig','self']:
            temp.append(i)
    return temp
        # if i[1] in []
...
def main(names,title,directory):
    ...
    main_keyword,text1=AnalyzeText(directory) #"./Text/temp.txt"등의 string
...
#main([],"","/home/joon/바탕화면/server_study/uploads/180830003557/List_dic_50000_Words.txt")
